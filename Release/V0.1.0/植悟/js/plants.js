...
// 植物库模拟数据（用于搜索）
export const plantLibraryData = [
    { name: '多肉', species: '多肉植物', category: '多肉', tags: ['耐旱', '易养护', '室内'] },
    { name: '绿萝', species: '天南星科', category: '观叶', tags: ['净化空气', '室内', '易养护'] },
    { name: '玫瑰', species: '蔷薇科', category: '开花', tags: ['观赏', '花香', '室外'] },
    { name: '龟背竹', species: '天南星科', category: '观叶', tags: ['室内', '观叶', '易养护'] },
    { name: '吊兰', species: '百合科', category: '观叶', tags: ['净化空气', '室内', '易养护'] },
    { name: '发财树', species: '木棉科', category: '绿植', tags: ['室内', '观叶', '寓意好'] },
    { name: '仙人掌', species: '仙人掌科', category: '多肉', tags: ['耐旱', '易养护', '室内外'] },
    { name: '文竹', species: '百合科', category: '观叶', tags: ['雅致', '室内', '喜阴'] },
    { name: '君子兰', species: '石蒜科', category: '开花', tags: ['观赏', '室内', '花期长'] },
    { name: '芦荟', species: '百合科', category: '多肉', tags: ['药用', '易养护', '室内'] }
];
...
function performSearch(keyword) {
    ...
    // 搜索我的植物
    results.myPlants = myPlants.filter(plant => 
        plant.nickname.toLowerCase().includes(keyword.toLowerCase()) ||
        plant.species.toLowerCase().includes(keyword.toLowerCase())
    );
    
    // 搜索植物库
    results.library = plantLibraryData.filter(plant =>
        plant.name.toLowerCase().includes(keyword.toLowerCase()) ||
        plant.species.toLowerCase().includes(keyword.toLowerCase()) ||
        plant.tags.some(tag => tag.toLowerCase().includes(keyword.toLowerCase()))
    );
...
function showSearchResults(results, keyword) {
    ...
    const { myPlants, library } = results;
    const hasMyPlants = myPlants.length > 0;
    const hasLibrary = library.length > 0;
    
    if (!hasMyPlants && !hasLibrary) {
        searchResults.innerHTML = `
            <div class="search-result-section">
                <div class="no-results">未找到与"${keyword}"相关的植物</div>
            </div>
        `;
    } else {
        searchResults.innerHTML = `
            ${hasMyPlants ? `
                <div class="search-result-section">
                    <div class="section-title">我的植物</div>
                    ${myPlants.map(plant => `
                        <div class="search-result-item" onclick="handleMyPlantClick('${plant.id}')">
            ...
            ${hasLibrary ? `
                <div class="search-result-section">
                    <div class="section-title">植物库</div>
                    ${library.map(plant => `
                        <div class="search-result-item" onclick="handleLibraryPlantClick('${plant.name}')">
                            <div class="plant-avatar">
...
// 处理我的植物点击
window.handleMyPlantClick = function(plantId) {
    hideSearchResults();
    showPlantDetailModal(plantId);
};

// 处理植物库植物点击  
window.handleLibraryPlantClick = function(plantName) {
    hideSearchResults();
    // 显示植物库植物详情或执行其他操作
    const plant = plantLibraryData.find(p => p.name === plantName);
    if (plant) {
        alert(`您点击了植物库中的 ${plant.name}，可以查看详细信息或添加到我的植物`);
        // 这里可以打开植物库详情弹窗
    }
};